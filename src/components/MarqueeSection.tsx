      ...
      <div className="flex flex-col gap-3">
        ...
        >
          {TRIPLED_ROW_1.map((src, index) => (
            <div
              key={`row1-${index}`}
              className="w-[420px] h-[270px] shrink-0 rounded-2xl overflow-hidden bg-[#16181d] border border-white/5"
            >
              <img
                src={src}
                alt={`3D showcase project ${(index % ROW_1_IMAGES.length) + 1}`}
                loading="lazy"
        ...
        >
          {TRIPLED_ROW_2.map((src, index) => (
            ...
            >
              <img
                src={src}
                alt={`3D showcase work ${(index % ROW_2_IMAGES.length) + 1}`}
                loading="lazy"
                referrerPolicy="no-referrer"
                className="w-full h-full object-cover rounded-2xl transition-transform duration-500 hover:scale-105"
              />
